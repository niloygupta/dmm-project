...
def gm_connected_components (num_nodes, node_table_name=GM_NODES, undirected_table_name=GM_TABLE_UNDIRECT):
    ...
    gm_sql_create_and_insert(db_conn, GM_CON_COMP, node_table_name, \
    ...
    while True:
        ...
        cur.execute("INSERT INTO %s " % temp_table + 
                            " SELECT node_id, MIN(component_id) \"component_id\" FROM (" +
                                " SELECT src_id \"node_id\", MIN(component_id) \"component_id\" FROM %s, %s" % (undirected_table_name,GM_CON_COMP) + 
    ...
    cur.execute ("SELECT node_id,component_id FROM %s" % GM_CON_COMP)
    components = cur.fetchall()
    for component in components:
        print "Node: ",component[0]," Component ID: ",component[1]
    cur.close()
